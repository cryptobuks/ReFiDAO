...
contract Treasury is Ownable {
     ...
     */
    constructor(
        address payable _walletAdr,
        address _extWalletAdr,
    ...
    /**
     *After a normal proposal is concluded, the beneficiary can trigger this function and
     ...
     */
    function startClearing(bytes32 _proposalID)
        public
        uponProposalSuccess(_proposalID)
        expectProposalType(_proposalID, false)
        returns (bool)
    {
        address _destination = proposalGate.getProposaldestinationAddress(
            _proposalID
        );
    ...
    /**
     *Member pay annual membership fee with this function
    ...
    /**
     *The beneficiary can request payment when proposal is successful and properly concluded.
     *@dev This action can only be taken when there's a delegate of the DAAS.
     */
    function withdrawMoneyFromInternalWallet()
        public
        allowPayout
        returns (bool)
    {
        return daoWallet.withdrawMoney(payable(msg.sender));
    }

    /**
     *The beneficiary can also withdraw money from the external wallet if possible
     *@dev This action can take place even the delegate is not in charge (no modifier of "allowPayout")
     *@param _proposalID the reference ID of proposals.
     */
    function withdrawMoneyFromExternalWallet(bytes32 _proposalID)
        public
        returns (bool)
    {
        return
            daoExternalWallet.withdrawMoney(_proposalID, payable(msg.sender));
    }

    /**
     *Change the address of wallets, upon success of proposal
     *@dev When the proposal is successful, anyone can trigger this action.
     *@notice Anyone can call this function.
     *@param _proposalID the reference ID of proposals.
     */
    function changeWalletAddress(bytes32 _proposalID)
        public
        uponProposalSuccess(_proposalID)
        returns (bool)
    {
        ...
        address payable _newInternalWalletAdr;
        address payable _newExternalWalletAdr;

        (_newInternalWalletAdr, _newExternalWalletAdr) = proposalGate
            .getProposalProposedWalletAddress(_proposalID);
        // require(msg.sender == address(proposalGate));
        require(
            _newInternalWalletAdr != address(0) ||
                _newExternalWalletAdr != address(0)
        );
        ...
        if (_newInternalWalletAdr != address(0)) {
        ...
        if (_newExternalWalletAdr != address(0)) {
    ...
    /**
     *Getter of member's contribution (membership fee/donation)
     *@param _adr the address of account
     */
    function getIndividualContribution(address _adr)
        public
        view
        returns (uint256)
    {
    ...
    /**
     *Getter of external funding of each external funds
     *@param _proposalID the reference ID of proposals.
     */
