...
contract MinimalProposal {
    ...
    modifier votable(bytes32 _proposalID) {
        require(
            block.timestamp.isInside(
                proposalList[_proposalID].startingTime,
                proposalList[_proposalID].endingTime
            )
        );
        _;
    }
    ...
    modifier concludable(bytes32 _proposalID) {
        require(
            block.timestamp.isFinished(proposalList[_proposalID].endingTime)
        );
        _;
    }
    ...
    modifier shouldLastNoLongerThan(
    ...
    {
        return
            block.timestamp.isInside(0, proposalList[_proposalID].endingTime);
    }
