...
contract GAManager is Ownable {
    ...
    modifier beforeGAstarts() {
        require(
            scheduledGA[currentIndex]
                .GAStartTime
                .add(scheduledGA[currentIndex].GADuration)
                .isFinished(block.timestamp)
        );
        _;
    }
    ...
    modifier shouldInSpan(uint256 _proposedGADate, bool _isExtraGA) {
        if (_isExtraGA) {
            require(
                _proposedGADate.isInside(block.timestamp, TIMESPAN_EXTRAGA)
            );
        } else {
    ...
    /**
     *Construct a GA Manger, who holds the information of current statutes
    ...
    /**
     *Update the address of the membership contract
    ...
    /**
     *Update address of the proposal manager contract
     *@dev This function can only be called by the DAA, eventually trigged by a successful proposal
    ...
    function setDelegateProposalsToGA(uint256 _gaIndex) public returns (bool) {
        uint256 _startingTime = getTimeIfNextGAExistsAndNotYetFullyBooked(
            _gaIndex
        );
        require(_startingTime != 0);
        scheduledGA[_gaIndex].delegateElectionTime = _startingTime;
        scheduledGA[_gaIndex].currentEndTime = _startingTime
            .add(VOTINGDURITION_PROPOSAL_GA)
            .add(VOTINGTIMEGAP_BETWEENPROPOSALS_GA);
        return true;
    }
     ...
     */
    function canSetGA(uint256 _time, bool _isExtraGA)
        public
        view
        shouldInSpan(_time, _isExtraGA)
        returns (bool)
    {
        uint256 minInterval;
        ...
        if (_time > block.timestamp.add(closestFuture)) {
            if (totalScheduledGA == 0) {
                return true;
            } else {
                uint256 i = currentIndex;
                while (i < totalScheduledGA) {
                    if (_time < scheduledGA[i].GAStartTime) {
                        if (
                            scheduledGA[i - 1]
                                .GAStartTime
                                .add(scheduledGA[i - 1].GADuration)
                                .add(minInterval) <
                            _time &&
                            _time.add(minInterval) < scheduledGA[i].GAStartTime
                        ) {
                            return true;
                        } else {
                            return false;
        ...
        if (
            _time > block.timestamp.add(closestFuture) &&
            scheduledGA[currentIndex]
                .GAStartTime
                .add(scheduledGA[currentIndex].GADuration)
                .add(minInterval) <
            ...
            if (
                totalScheduledGA > currentIndex &&
                _time.add(minInterval) <
                scheduledGA[currentIndex + 1].GAStartTime
            ) {
                return true;
            }
        }
     ...
     */
    function concludeDelegateVoting(uint256 _minParticipant, uint256 _minYes)
        public
        returns (bool, bool)
    {
        ...
        if (
            ...
            if (
                ...
                if (potentialCandidateListForCurrentGA.revoteOrNot == false) {
                    ...
                    if (
                        temp.GAStartTime.canSchedule(
                            temp.GADuration,
                            temp.currentEndTime,
                            VOTINGDURITION_PROPOSAL_GA
                        ) == false
                    ) {
                        // current GA is full. Need to extend it and shcedule the meeting
                        scheduledGA[currentIndex].GADuration = scheduledGA[
                            currentIndex
                        ].GADuration.add(VOTINGDURITION_PROPOSAL_GA);
                    }
                    // put the next round into the current GA
                    scheduledGA[currentIndex]
                        .delegateElectionTime = scheduledGA[currentIndex]
                        .currentEndTime;
                    scheduledGA[currentIndex].currentEndTime = scheduledGA[
                        currentIndex
    ...
    function isDuringGA() public view returns (bool) {
        if (
            block.timestamp.isInside(
                scheduledGA[currentIndex + 1].GAStartTime,
                scheduledGA[currentIndex].GAStartTime.add(
                    scheduledGA[currentIndex].GADuration
                )
            )
        ) {
            return true;
    ...
    function updateCurrentGA() public returns (bool) {
        if (
            block.timestamp.isInside(
                scheduledGA[currentIndex + 1].GAStartTime,
                scheduledGA[currentIndex + 1].GAStartTime.add(
                    scheduledGA[currentIndex + 1].GADuration
                )
            )
        ) {
            currentIndex++;
     ...
     */
    function setNewStatute(bytes32 _proposalID)
        public
        successfulProposal(_proposalID)
        returns (bool)
    {
    ...
    /**
     *check If the next GA is planned. If yes, whether is already fully booked.
     ...
     */
    function getTimeIfNextGAExistsAndNotYetFullyBooked(uint256 _gaIndex)
        public
        proposalOnly
        returns (uint256)
    {
        ...
        if (
            temp.GAStartTime > block.timestamp &&
            temp.GAStartTime.canSchedule(
                temp.GADuration,
                temp.currentEndTime,
                VOTINGDURITION_PROPOSAL_GA
            ...
            scheduledGA[_gaIndex].currentEndTime = _end
                .add(VOTINGDURITION_PROPOSAL_GA)
                .add(VOTINGTIMEGAP_BETWEENPROPOSALS_GA);
            return _end;
        } else {
    ...
    /**
     *Getter to check if we could vote for delegate now
     */
        ...
        if (candidateAssitant.listOfCandidateAddress[_candidate] != 0) {
            return (
                block.timestamp.isInside(
                    ...
                    temp.delegateElectionTime.add(VOTINGDURITION_PROPOSAL_GA)
                )
            );
        } else {
            return false;
    ...
    // /**
    //  *Check if there is already some time scheduled for delegate election. If not set it.
    ...
    // /**
    //  *This function updates the current schedule of a given GA.
    ...
    /**
     *Getter for the starting time and the duration of the GA
     *@param _gaIndex The index of the addressed scheduled GA.
     */
    function getGAStartAndDuration(uint256 _gaIndex)
        public
        view
