...
library TallyClerkLib {
    ...
    /**
     *To refresh and calculate the result.
     *@notice This function should be called, everytime a member votes.
     */
    function refreshResult(
        VotesPerProposal storage _currentVotes,
        address _voter,
        voteTicket _newVote
    ) public {
        if (
            _currentVotes.participantList[_voter] ==
            voteTicket.NotYetParticipated
        ) {
            // new vote
            ...
            if (_currentVotes.participantList[_voter] == voteTicket.Yes) {
                ...
                _currentVotes.participantList[_voter] == voteTicket.Abstain
            ) {
    ...
    /**
     *Tell whether the result of a voting wins majority or not
     *@notice For even number, the positive votes should exceed half of the received votes.
     */
    function isMajority(uint256 _yesVoteNum, uint256 _totalNum)
        public
        pure
        returns (bool)
    {
    ...
    /**
     *Calculate whether a quorum of voting has reached or not.
     *@dev   simple comparison...
     */
    function calculateQuorum(
        uint256 _totalNumber,
        uint256 _quorumNumerator,
        uint256 _quorumDemoninator
    ) public pure returns (uint256) {
        require(_quorumNumerator <= _quorumDemoninator);
        uint256 _quorum = _totalNumber.mul(_quorumNumerator).div(
            _quorumDemoninator
        );
        return _quorum;
    }
    ...
    /**
     *Function to find who are the candidates who hold the largest votes, in the election for new delegate
     ...
     */
    function findMostVotes(CandidancyForDelegate storage _allCandidates)
        public
    {
        uint256 temp = 0;
        uint256 copies = 0;
    ...
    /**
     *Function tells whether a revote is needed or not
     *@dev well... A public view function
     */
    function needRevote(CandidancyForDelegate storage _allCandidates)
        public
        view
        returns (bool)
    {
